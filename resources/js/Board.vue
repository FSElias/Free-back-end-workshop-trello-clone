  ...
  <div class="h-full flex flex-col items-stretch" :class="bgColor">
    ...
    <div class="h-full flex flex-1 flex-col items-stretch">
      ...
      <div class="flex flex-1 items-start overflow-x-auto mx-2" v-if="board">
        <List
          :card_list="card_list"
          v-for="card_list in board.card_lists"
          :key="card_list.id"
...
import BoardQuery from "./graphql/BoardWithCardListsAndCards.gql";
...
export default {
  ...
  methods: {
    ...
    updateQueryCache(event) {
      ...
      const card_listById = () =>
        data.board.card_lists.find(card_list => card_list.id == event.cardListId);

      switch (event.type) {
        case EVENT_LIST_ADDED:
          data.board.card_lists.push(event.data);
          break;
        case EVENT_CARD_ADDED:
          card_listById().cards.push(event.data);
          break;
        case EVENT_CARD_UPDATED:
          card_listById().cards.filter(card => card.id == event.data.id).title =
            event.data.title;
          break;
        case EVENT_CARD_DELETED:
          card_listById().cards = card_listById().cards.filter(
